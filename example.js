...
const cherryPickCommitsOnRef = async ({
    ...
    const {data: {tree: {sha: initialHeadTree},},} = await octokit.git.getCommit({
        commit_sha: initialHeadSha,
        owner,
        repo,
    });

    const {sha: newHeadSha} = await commits.reduce(async (previousCherryPick, commit) => {
        const {
            sha,
            tree
        } = await previousCherryPick;
